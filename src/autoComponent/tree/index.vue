<template>
  <div onselectstart="return false">
    <draggable v-model="nodes" :disabled="!editable || !sortable" @end="onDragEnd" :group="dragKey">
      <transition-group>
        <template v-if="nodes && nodes.length > 0" v-for="(node, index) of nodes">
          <div class="tree-node list-group-item" :key="index + '_nodes'"
            ...
            <span class="icon editable el-icon-plus" style="right: 33px" @click="beginAdd($event, node)"
                  v-if="editable && children"></span>
          </div>
          <items :datas="node.nodes" :selected="selected" :multiple="multiple" :on-selected="onSelected"
                 :on-unselected="onUnselected" :key="'tree_item_' + index"
                 :on-selection-changed="onSelectionChanged" :level="level + 1" v-show="node.open" :id="id" :text="text"
                 :editable="editable" :on-create="onCreate" :on-update="onUpdate"
                 v-if="isFolder(node)"></items>
        </template>
      </transition-group>
    </draggable>
    <div v-if="level === 1 && editable" class="tree-node list-group-item" style="text-align: center;"
         @click="beginAdd($event)">
      <span class="el-icon-plus">{{ addText }}</span>
    </div>
  </div>
</template>
/**
    ...
    onClick(node) {
      ...
      if (this.isSelected(node)) {
        ...
        if (this.onSelectionChanged) {
          this.onSelectionChanged(this.selected.filter(s => s != null), node)
        }
      }
    }
  },
