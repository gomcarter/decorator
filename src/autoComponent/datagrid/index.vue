/* eslint-disable */
<template>
  <table class="datagrid-container datagrid-frame" :style="{'width': width ? width : '99.9%'}">
    <thead class="datagrid-header">
      <tr v-if=" (title || '').trim().length > 0" class="datagrid-title">
        <th :colspan="columns.length + checkable + (+!!children)">
          <p class="datagrid-title-text">{{ title }}</p>
        </th>
      </tr>
      <tr v-if="toolbar && toolbar.length > 0" class="datagrid-toolbar">
        <th :colspan="columns.length + checkable + (+!!children)">
          <el-button
            ...
            size="medium"
            :disabled="(item.disabled && item.disabled()) || t*0 !== 0"
            @click="item.handler && item.handler()">{{ item.title }}
          </el-button>
        </th>
      </tr>
      <tr v-if="columns != null && columns.length > 0" class="datagrid-header-row">
        <th v-if="children" class="datagrid-row-expander-all">
          <span :class="{'el-icon-plus': !expandAll, 'el-icon-minus': expandAll}" @click="expandAllToggle"></span>
        </th>
        <th v-if="checkable" class="datagrid-header-check">
          <el-checkbox v-if="!singleCheck && t" v-model="checkAll" :disabled="(data || []).length === 0"></el-checkbox>
        </th>
        <th
          v-for="(column, index) in columns"
          :key="index"
          class="datagrid-header-cell"
          :style="{ width: column.width > 0 ? (column.width + 'px') : column.width }"
          @click="onHeaderClicked(column)">
          {{ column.header }}
          <span
            :class="{'el-icon-caret-top' : sort === column.sort && (order ||'').toUpperCase() === 'ASC', 'el-icon-caret-bottom': sort === column.sort && (order ||'').toUpperCase() === 'DESC', 'el-icon-d-caret': column.sort && sort !== column.sort}"></span>
        </th>
      </tr>
    </thead>
    <tbody class="datagrid-body">
      <template
        ...
        <tr class="datagrid-body-row" :key="'r_' + rowIndex" @dblclick="expandRowToggle(row)" @click="onRowClicked(row)">
          <td v-if="children" class="datagrid-row-expander">
            <span
              :class="{'el-icon-plus': !row.$expanded, 'el-icon-minus': !!row.$expanded}"
              @click="expandRowToggle(row)">
            </span>
          </td>
          <td v-if="checkable" class="datagrid-row-check">
            <el-checkbox v-model="row.$checked" @change="onRowCheckChanged(row)"></el-checkbox>
          </td>

          <td v-for="(col, colIndex) in columns" :key="colIndex" class="datagrid-body-td"
              :style="{ width: col.width > 0 ? (col.width + 'px') : col.width }">
            <div v-if="editable && col.editable" class="datagrid-body-cell">
              <el-date-picker v-if="col.editType === 'date'" v-model="row[col.field]" type="date" size="medium"
                              placeholder="选择日期" class="datagrid-editable no-suffix no-prefix"
                              @change="editValueChanged(col, row, rowIndex)"></el-date-picker>
              <ac-selector v-else-if="col.editType === 'selector'" v-model="row[col.field]" placeholder="选择"
                           :data="typeof col.editSelections === 'function' ? col.editSelections(row, rowIndex) : col.editSelections"
                           :url="col.editUrl" :filterable="!!col.editUrl" size="medium"
                           class="datagrid-editable no-suffix no-prefix"
                           :on-selection-changed="editValueChanged(col, row, rowIndex)"></ac-selector>
              <el-input v-else v-model="row[col.field]" class="datagrid-editable"
                        @input="editValueChanged(col, row, rowIndex)"></el-input>
            </div>
            <div v-else-if="col.html" class="datagrid-body-cell" v-html="computeValue(col, row, rowIndex)"></div>
            <div v-else class="datagrid-body-cell"> {{ computeValue(col, row, rowIndex) }}</div>
            <div v-if="col.actions != null && col.actions.length > 0" class="datagrid-body-cell">
              <a
                v-for="(action, index) in col.actions"
                v-if="action.show != null ? action.show(row) === true: true"
                :key="'datagrid-action-' + index"
                class="datagrid-action"
                @click="action.handler != null && action.handler(row, rowIndex)">
                <el-badge
                  :value="action.badge && action.badge.value(row, rowIndex)"
        ...
        <tr :key="'d_' + rowIndex" class="datagrid-body-row-child" v-if="children && row.$expanded">
          <td v-if="children" class="datagrid-body-td"></td>
          <td v-if="checkable" class="datagrid-body-td"></td>
          <td class="datagrid-body-td" :colspan="columns.length">
            <ac-datagrid
              :width="children.width"
              :checkable="children.checkable"
              :single-check="children.singleCheck"
              :pageable="children.pageable"
              :title="children.title"
              :sort-by="children.sortBy"
              :order-by="children.orderBy"
              :toolbar="children.toolbar"
              :columns="children.columns"
              :data-url="children.dataUrl"
              :load-data="children.loadData"
              :on-before-load="children.onBeforeLoad"
              :count-url="children.countUrl"
              :params="(children.params && children.params(row, rowIndex)) || {}">
            </ac-datagrid>
          </td>
        </tr>
      </template>
      <tr v-if="!data || data.length === 0" class="datagrid-body-row nodata">
        <td class="datagrid-body-td" :colspan="columns.length + checkable + (+!!children)">暂无数据</td>
      </tr>
      <tr v-if="pageable" class="datagrid-footer">
        <td :colspan="columns.length + checkable + (+!!children)">
          <ac-pager :page="page" :total="total" :rows="rows" @onChanged="onPagerChanged"></ac-pager>
        </td>
      </tr>
    </tbody>
  </table>
...
* editable:Boolean    - 是可编辑，必须column上和这里一起true，才可以编辑
...
*                     -  formatter(value, index, row) { 如果返回值需要格式化，可以使用此方法。比如时间戳转时间字符串等等，可以返回html
*                     -   return value
*                     -  },
*                     -  editable: true -- 是否支持编辑
*                     -  editType: 'date', 'selector' 默认文本编辑，支持日期，下拉框
*                     -  editSelections： 下拉选项； editUrl——下来远程选项
...
* onRowClicked(row)             - 用户点击某一行时，回调此方法，参数：选中行。
...
export default {
  ...
  methods: {
    ...
    editValueChanged(col, row, index) {
      // 其中某个编辑的变量变化了，调用所有formatter
      this.columns.forEach(v => {
        if (v.formatter) {
          this.computeValue(v, row, index)
        }
      })
    },
    computeValue(col, row, index) {
      ...
      if (formatter) {
        value = formatter(value, index, row)
      } else if (col.map) {
        value = col.map[value]
      }
      return value
